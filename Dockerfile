...
USER ows
RUN npm install -g @wedontknowjs/bch-node@0.0.11

WORKDIR $HOME_PATH
RUN $PKG_NAME create -d $BITCOIN_DATA $APP_NAME

WORKDIR $APP_DIR
RUN $PKG_NAME install @wedontknowjs/bch-explorer-api@0.0.7
RUN $PKG_NAME install @wedontknowjs/bch-wallet-service@0.0.9
RUN $PKG_NAME install @wedontknowjs/ows-explorer@0.0.3
RUN $PKG_NAME install @wedontknowjs/ows-elastic-sync@1.0.3
