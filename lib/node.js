...
Node.prototype.getServiceOrder = function() {
  ...
  var moduleNames = [];
  var servicesByModuleName = {};
  for (var i = 0; i < services.length; i++) {
    var service = services[i];
    moduleNames.push(service.moduleName);
    servicesByModuleName[service.moduleName] = service;
  }

  var stackNames = {};
  var stack = [];

  function addToStack(moduleNames) {
    for(var i = 0; i < moduleNames.length; i++) {

      var moduleName = moduleNames[i];
      var service = servicesByModuleName[moduleName];
      $.checkState(service, 'Required dependency "' + moduleName + '" not available.');

      // first add the dependencies
      addToStack(service.module.dependencies);

      // add to the stack if it hasn't been added
      if(!stackNames[moduleName]) {
        stack.push(service);
        stackNames[moduleName] = true;
      }

    }
  }

  addToStack(moduleNames);
 ...
 * @param {String} serviceInfo.moduleName - The loadable module name of the service
...
Node.prototype._startService = function(serviceInfo, callback) {
  ...
  log.info('Starting ' + serviceInfo.moduleName +
    (serviceInfo.moduleName !== serviceInfo.name ? ' as ' + serviceInfo.name : ''));
  ...
  if (serviceInfo.config) {
    ...
    $.checkState(!serviceInfo.config.moduleName);
  ...
  config.moduleName = serviceInfo.moduleName;
  ...
  async.eachSeries(
    ...
    function(service, next) {
      if (self.services[service.name]) {
        log.info('Stopping ' + service.moduleName +
          (service.moduleName !== service.name ? ' running as ' + service.name : ''));
        self.services[service.name].stop(next);
      } else {
        log.info('Stopping ' + service.moduleName +
          (service.moduleName !== service.name ? ' running as ' + service.name : '') + ' (not started)');
        setImmediate(next);
      }
    },
    callback
