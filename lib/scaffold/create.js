...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with Bch',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@wedontknowjs/bch-lib': '^' + bchLib.version,
    '@wedontknowjs/bch-node': version
  }
};
...
/**
 * Will create a base Bch Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bch-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Bch Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the bch node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bch-node directory and configuration
