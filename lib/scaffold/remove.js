...
/**
 * Will remove a service from bch-node.json
...
/**
 * Will remove the Node.js service and from the bch-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The bch-node configuration directory
...
function remove(options, done) {
  ...
  var bchNodeConfigPath = path.resolve(configPath, 'bch-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bchNodeConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a bch-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to bch-node.json
        removeConfig(bchNodeConfigPath, service, next);
      });
    }, done
  );
}
