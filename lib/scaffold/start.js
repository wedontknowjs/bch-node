...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and bch-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/owstack/bch-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed bch-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bch-node.json');
...
function checkService(service) {
  ...
  if (!service.module.prototype ||
    ...
    throw new Error(
      'Could not load service ' + service.name +
        (service.moduleName !== service.name ? ' using module name ' + service.moduleName : '') +
...
function loadModule(req, service) {
  try {
    // first try in the built-in bch-node services directory
    service.module = req(path.resolve(__dirname, '../services/' + service.moduleName));
  } catch(e) {

    // check if the package.json specifies a specific file to use
    var servicePackage = req(service.moduleName + '/package.json');
    var serviceModule = service.moduleName;

    // The package.json should not define both bchNode and owsNode
    if (servicePackage.bchNode) {
      serviceModule = service.moduleName + '/' + servicePackage.bchNode;
    } else if (servicePackage.owsNode) {
      serviceModule = service.moduleName + '/' + servicePackage.owsNode;
    }
...
function setupServices(req, servicesPath, config) {
  ...
  if (config.services) {
    for (var i = 0; i < config.services.length; i++) {
      ...
      service.moduleName = service.config.module || service.name;
 ...
 * @param {Node} node - The Bch Node instance
 */
