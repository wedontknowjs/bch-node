...
var BitcoinRPC = require('@wedontknowjs/bitcoind-rpc');
...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    if (bchLib.Address.isValid(addresses[i], this.node.network)) {
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bch-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bch-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bch-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from bch-node. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of bch-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === bchLib.Networks.testnet) {
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = bchLib.crypto.Hash.sha256sha256(message);
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    node.client = new BitcoinRPC({
      protocol: self.spawn.config.rpcprotocol,
 ...
 * @param {String|Address|Array} addressArg - An address string, bch address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = bchLib.Script.fromAddress(delta.address);
    return {
...
Bitcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in bch is most recent to less recent
...
Bitcoin.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof bchLib.Address) {
...
/**
 * Will retrieve a block as a Bch object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = bchLib.Block.fromString(response.result);
...
/**
 * Will get a transaction as a Bch Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
